...
export default function useLitLibrary() {
	...
	function getAccessControlConditions(contractAddress) {
	...
	async function executeLitAction(contractAddress, code, args) {
	...
	async function encryptData(dataToEncrypt, contractAddress) {
		let litNodeClient = client
		if (!litNodeClient) {
			litNodeClient = await getLitNodeClient()
		}
		...
		const accessControlConditions = !contractAddress
			? getAccessControlConditions2()
			: getAccessControlConditions(contractAddress)
		// 1. Encryption
	...
	async function encryptFile(fileToEncrypt, contractAddress) {
		let litNodeClient = client
		if (!litNodeClient) {
			litNodeClient = await getLitNodeClient()
		}
	...
	async function decryptData(ciphertext, dataToEncryptHash, contractAddress) {
		...
		try {
			decryptedString = await LitJsSdk.decryptToFile(
				{
					authSig: sig,
					accessControlConditions: !contractAddress
						? getAccessControlConditions2()
						: getAccessControlConditions(contractAddress),
					ciphertext,
					dataToEncryptHash,
	...
	const testEncData = async (message, sessionSigs, litNodeClient) => {
		const { ciphertext, dataToEncryptHash } = await LitJsSdk.encryptString(
			{
	...
	const requestWeb3Storage = async (did) => {
		const api_url_ciphertext = process.env.NEXT_PUBLIC_API_URL_CIPHERTEXT
		const api_url_datatoencrypthash =
			process.env.NEXT_PUBLIC_API_URL_DATA_TO_ENCRYPT_HASH

		const api_key_ciphertext = process.env.NEXT_PUBLIC_API_KEY_CIPHERTEXT
		const api_key_datatoencrypthash =
			process.env.NEXT_PUBLIC_API_KEY_DATA_TO_ENCRYPT_HASH

		const litNodeClient = await getLitNodeClient()

		console.log(
			'starting to request web3 storage credentials through lit actions'
		)
		...
		console.log(
			'execute js using encrypted api key and url to hide details of where user gets granted information'
		)
	...
	const testEnc = async () => {
		...
		const { ciphertext, dataToEncryptHash } = await testEncData(
			messageToEncrypt,
			sessionSigs,
			litNodeClient
		)
		...
		console.log(
			'api key ciphertext and data to encrypt',
			ciphertext,
			dataToEncryptHash
		)
		...
		const { ciphertext: ct2, dataToEncryptHash: dt2 } = await testEncData(
			messageToEncrypt2,
			sessionSigs,
			litNodeClient
		)
		...
		const decrypted = await testDecData(
			ct2,
			dt2,
			ciphertext,
			dataToEncryptHash,
